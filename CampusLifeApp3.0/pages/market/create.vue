<template>
  <view class="page">
    ...
    <u-navbar title="发布闲置" :autoBack="true" bgColor="transparent" leftIconColor="#1E293B" titleStyle="color: #1E293B; font-weight: 700;" placeholder></u-navbar>
    ...
    <scroll-view scroll-y class="content">
      <view class="form-container animate-enter" style="--delay: 0s">
        ...
        <view class="section-card">
          ...
          <text class="tip-text">第一张图片将作为封面展示</text>
        </view>
        ...
        <view class="section-card animate-enter" style="--delay: 0.2s">
          ...
          <view class="form-group">
            <view class="textarea-wrapper">
              <u-textarea
                  v-model="form.desc"
                  placeholder="描述一下物品的入手渠道、转手原因、具体瑕疵等..."
                  border="none"
                  height="140"
                  count
                  maxlength="500"
...
import { createMarketItem, getMarketCategories } from '@/api/market.js'
import { uploadFile } from '@/api/request.js'

const cats = ref([])
const categoryList = ref([]) // 存储分类完整数据
const conditions = ['全新', '几乎全新', '轻微使用痕迹', '明显使用痕迹', '功能故障']
...
async function submit(){
  if (!form.title || !form.price || !form.category || !form.condition || form.images.length === 0){
