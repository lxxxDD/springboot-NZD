...
### 2.13 AI对话会话表 (chat_sessions)
```sql
CREATE TABLE chat_sessions (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  session_id VARCHAR(64) UNIQUE NOT NULL COMMENT '会话ID',
  user_id BIGINT NOT NULL,
  title VARCHAR(100) COMMENT '会话标题(取自首条消息)',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX idx_user (user_id),
...
### 3.9 AI 助手模块 `/api/chat`

| 方法 | 路径 | 描述 | 请求参数 | 响应 |
|------|------|------|----------|------|
| POST | /send | 发送消息获取AI回复 | `{message, sessionId?}` | `{code, data: {reply, sessionId}}` |
| GET | /history | 获取对话历史 | `?sessionId=&page=&size=` | `{code, data: {list, hasMore}}` |
| DELETE | /history/:sessionId | 清除对话历史 | - | `{code, message}` |
| GET | /suggestions | 获取推荐问题 | - | `{code, data: {suggestions: []}}` |
...
### 7.6 AI助手页面 `pages/chat/chat.vue`
```javascript
// TODO: 替换为真实 AI 对话 API
// import { sendChatMessage } from '@/api/chat'
// const res = await sendChatMessage(text.value, sessionId.value)
// msgs.value.push({ sender: 'ai', text: res.data.reply })
```
...
推荐使用以下 AI 服务之一：
- **OpenAI API** - ChatGPT (需要国际网络)
- **百度文心一言** - 国内可用，有免费额度
- **阿里通义千问** - 国内可用，有免费额度
- **讯飞星火** - 国内可用，有免费额度

### 8.2 后端 Controller 示例
...
public class ChatController {
    ...
    public Result<ChatResponse> sendMessage(@RequestBody ChatRequest request,
        ...
        ChatResponse response = chatService.chat(userId, request.getMessage(), request.getSessionId());
        return Result.success(response);
    }
    
    @GetMapping("/history")
    public Result<PageResult<ChatMessage>> getHistory(@RequestParam String sessionId,
                                                       @RequestParam(defaultValue = "1") Integer page,
                                                       @RequestParam(defaultValue = "20") Integer size) {
        return Result.success(chatService.getHistory(sessionId, page, size));
    }
...
public class ChatServiceImpl implements ChatService {
    ...
    @Autowired
    private CampusKnowledgeMapper knowledgeMapper;
    ...
    public ChatResponse chat(Long userId, String message, String sessionId) {
        ...
        // 5. 调用 AI API
        String reply = callQwenAPI(systemPrompt, history, message);
    ...
    private String callQwenAPI(String systemPrompt, List<ChatMessage> history, String message) {
        // 调用通义千问 API
        // 具体实现参考阿里云官方文档
...
```sql
-- 插入校园知识库初始数据
INSERT INTO campus_knowledge (category, question, answer, keywords) VALUES
('library', '图书馆开放时间', '图书馆开放时间：周一至周五 8:00-22:00，周六周日 9:00-21:00。考试周期间延长至23:00。', '图书馆,开放,时间,几点'),
('library', '如何借书', '借书流程：1. 在图书馆检索系统查找图书位置 2. 到对应楼层书架取书 3. 到自助借还机或服务台办理借阅 4. 本科生可借10本，借期30天', '借书,借阅,怎么借'),
('canteen', '食堂营业时间', '食堂营业时间：早餐 6:30-9:00，午餐 11:00-13:00，晚餐 17:00-19:30。夜宵档口营业至21:30。', '食堂,营业,时间,几点'),
('canteen', '食堂推荐', '推荐菜品：一食堂的红烧肉、二食堂的麻辣香锅、三食堂的水煮鱼都很受欢迎！', '食堂,推荐,好吃,美食'),
('repair', '如何报修', '报修流程：1. 打开校园生活APP 2. 进入"报修"页面 3. 选择报修类型和位置 4. 描述问题并上传照片 5. 提交等待处理', '报修,维修,怎么报'),
('activity', '如何参加活动', '参加活动：1. 在首页或活动页面浏览活动 2. 点击感兴趣的活动查看详情 3. 点击"立即报名"按钮 4. 按时参加活动即可', '活动,报名,参加'),
('general', '校医院位置', '校医院位于学校东门附近，24小时急诊服务。普通门诊时间：周一至周五 8:00-17:00。', '校医院,医院,看病,位置');
```
...
2. **数据库设计** - 15 张核心表的完整 SQL（含 AI 对话相关表）
3. **API 接口设计** - 9 个模块共 45+ 个接口（含 AI 助手模块）
4. **前端 API 封装** - 请求工具和各模块 API
5. **WebSocket 配置** - 实时消息方案
6. **后端技术栈** - Spring Boot 项目结构和依赖
7. **接口预留位置** - 方便后续对接
8. **AI 助手实现方案** - 完整的后端代码示例和知识库设计
